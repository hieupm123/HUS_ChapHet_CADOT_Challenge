...
def modify_coco_annotations(input_coco_file, output_coco_file):
    ...
    target_place_names = [
        "basketball field", "football field", "graveyard",
        "playground", "roundabout", "swimming pool", "tennis court"
    ]
    target_place_ids = [category_name_to_id[name] for name in target_place_names if name in category_name_to_id]

    target_vehicle_names = ["large vehicle", "medium vehicle"]
...
input_file_2 = "../data/train_inpainted_cv2/_annotations.coco.json"
# Đặt tên file output, ví dụ: thêm "_modified" vào tên file gốc
output_file_parts_2 = input_file_2.split('.')
if len(output_file_parts_2) > 2 and output_file_parts_2[-2] == "coco": # Xử lý trường hợp .coco.json
    output_file_2 = ".".join(output_file_parts_2[:-2]) + "_modified.coco.json"
else: # Xử lý trường hợp .json
    output_file_2 = ".".join(output_file_parts_2[:-1]) + "_modified.json"
...
modify_coco_annotations(input_file_2, output_file_2)
